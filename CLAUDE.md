...
**v1.1.0 — tagged and released (2026-08-03). Tests: 545 passed.**
...
- `app/glossary.py` — Markdown-table glossary at `{vault}/FuseMark/Glossary.md`, build Whisper prompt, add terms (pipe/newline-escaped, alias-aware dedup), open in Obsidian/default handler
...
- v1.0.2 milestone (2026-07-11): closed all 9 issues (#51, #53–#60) across 4 branches/PRs, ordered by severity (data loss first, docs last). **Data safety** (#64, merged, closes #56/#57): `notes.save_note()`/`save_transcript()` gained an `existing_path` param — a retry of the same job overwrites its own prior output, a genuine same-day/same-label collision from a different job is uniquified (` (2)`, ` (3)`, …) instead of clobbered; `config.py` `save()` now writes via temp-file + `os.replace()`, `load()` recovers from a corrupt file (backs it up, returns defaults) instead of crashing at startup, and a new `cfg.lock()` context manager protects the one real race — `updater.check_for_update()` vs `/settings/save`. **Recording lifecycle** (#65, merged, closes #54): `RecordingService.stop()`/`start()`, `Recorder.start()`, and `main._quit()` all guard their failure paths — a `save()` failure marks the job `error` instead of stranding it, a mic-open failure cleans up the already-open loopback stream instead of leaking it, and a failed flush no longer aborts app teardown. **LLM pipeline** (#66, merged, closes #51/#53/#55/#58): `worker._generate()` now loads the user's picked vault template via `notes.load_template()` (previously wired to nothing — zero callers) and threads it through `generate_notes(custom_template=...)` in all three providers as a plain "use this note structure" line, without touching the `note_system.txt` prompt-file contract; a new `LLMTransientError` (connection/5xx/overloaded) joins `LLMRateLimitError` as retryable in all three providers + `worker._generate()`; `/jobs/<id>/retry` now resets `retry_count` to 0; note-generation `max_tokens` raised 4096→8192 with a new `LLMTruncatedError` raised on `stop_reason`/`finish_reason == "length"` instead of silently saving a cut-off note. **Minor bundle** (#67, merged, closes #59): `/jobs/<id>/audio` 404s on an unknown job and restricts audio deletion to `done`/`error` jobs (marking "keep" is unaffected); `/wizard/test-recording` guards against a concurrent real recording (409) and cleans up on failure instead of leaking a stream; the dead `extra_context === 'transcribing:uploading'/'transcribing:processing'` comparison (pre-#47 progress hack) removed from `static/app.js`; `/update-check` no longer reimplements `updater.check_for_update()`'s fetch/parse — both share a new `updater._fetch_latest_release()`, each keeping the error-handling shape its caller needs. Also fixed CLAUDE.md's D1 doc drift (#60): intro/tech-stack table now say Haiku 4.5 (+ OpenAI/Mistral), `pyaudiowpatch`, `large-v3-turbo`, any Markdown vault. Tests: 427 passed. `app/version.py` and `installer/setup.iss` `MyAppVersion` bumped together to 1.0.2 in #68 (CI enforces they match — see `test_setup_iss_version_matches_app_version`). All 4 PRs merged; tag `v1.0.2` pushed, CI release workflow passed (tests + PyInstaller build), and the installer was built and published locally by Claude (Inno Setup already installed from the v1.0.1 session). **Caught a near-repeat of the v1.0.0 ffmpeg double-bundling bug**: `ffmpeg.exe`/`ffprobe.exe` had been copied into `dist/FuseMark/` for a local smoke-test launch (per `installer/README.md`'s documented steps) and left there — `setup.iss` bundles both `dist/FuseMark/*` and `installer/ffmpeg/*` separately, so the installer size (174 MB) was ~50 MB over v1.0.1 before the leftover copies were removed and it was rebuilt (125.1 MB, matching v1.0.1 almost exactly). The documented size-comparison check in `installer/README.md` is what caught it — worth keeping as a mandatory step, not just a suggestion. Released unsigned, same as v1.0.0/v1.0.1 (no code-signing cert yet — still maintainer-only).

- v1.1 milestone (2026-08-02): 8 issues across 8 PRs/branches, sequenced to avoid file conflicts (recorder pair first since both rewrite the same capture callbacks; glossary hardening before the UI that gives `add_terms()` its first caller; the two prompt-engine issues as one PR since both live in the same function region). **Structure** (#18, PR #69): `app/server.py` (820 lines/39 routes) split into 8 Flask Blueprints under `app/routes/`, verified as a pure move by diffing the pre/post `url_map` — zero existing tests modified. **Recorder** (#52, PR #71; #43, PR #73): `Recorder` streams to two partial WAV files under `recordings/.partial/<job_id>.{system,mic}.wav` instead of buffering a whole meeting in RAM; a crash mid-recording is now recoverable via `salvage_partial()`/`rewrite_wav_header()` at startup; the level meter polls real RMS from `GET /level` every 250ms instead of animating `Math.random()`, with a mic-silence warning once `SILENCE_SECONDS` passes with no signal. **Glossary** (#35, PR #74; #44, PR #75): `add_terms()` hardened (pipe/newline escaping, malformed-LLM-output guards, alias-aware dedup) before giving it its first production caller — a new `POST /jobs/<id>/glossary` route + jobs-panel checkboxes let a user approve/dismiss the suggestions the worker already computes; a new `glossary_suggestions` config flag makes that per-job LLM call actually toggleable. **Prompt engine** (#31 + #30, PR #76): `_substitute()` rewritten as a single simultaneous regex pass so a value substituted for one placeholder can't have its own literal `{other_key}` text re-expanded by a later pass; `_load()` gained an in-process cache keyed on a stat fingerprint of both the user-override and bundled-default files, so creating/editing/deleting an override all correctly invalidate it. **Templates** (#27, PR #77): vault templates can now carry an optional `<!-- fusemark:prompt -->` block with extra LLM instructions, parsed out and appended to the system prompt via a new `app/llm/_common.py` `build_system_prompt()` — deliberately not folded into `note_system.txt` as a placeholder, which would break every existing user override; also fixed doc drift in `docs/template_example.md` (vault templates are not `{date}`/`{title}`-substituted, only FuseMark's built-in one is). `app/version.py` and `installer/setup.iss` `MyAppVersion` bumped together to 1.1.0. Tests: 543 passed.

**Release (2026-08-03):** tag `v1.1.0` pushed, CI release workflow passed (tests + PyInstaller build). Installer built and published locally by Claude (Inno Setup already installed from prior sessions): packaged exe smoke-tested standalone (Flask served, ffmpeg detected, no DLL-load errors) after briefly stopping the user's own running dev instance (`python -m app.main`, restarted by the user afterward is up to them) to free the single-instance mutex/port 5000; ffmpeg copies used only for the smoke test were removed from `dist/FuseMark/` before packaging, avoiding a repeat of the v1.0.0/v1.0.2 double-bundling bug. Installer size 125.15 MB vs. v1.0.2's 125.14 MB — no regression. Released unsigned, same as prior versions (no code-signing cert yet — still maintainer-only). Local `build/`, `dist/`, `installer/Output/` cleaned up after publishing. **Maintainer still needs to**: do the manual hardware pass on the recorder/level-meter noted in PR #71/#73 (not yet confirmed on real hardware).
...
| Glossary | Markdown table at `{vault}/FuseMark/Glossary.md` |
